...
function writeFile(file, folder, name) {
  ...
  fs.writeFile(`./releasescripts/packs/${folder}/_source/${name}.json`, file, err => {
    if (err) {
...
function createFile(scripts) {
  ...
  for (i = 0; i < scripts.length; i++) {
    ...
    writeFile(JSON.stringify(filecontents, null, 4), "SR5-Community-Macros", name);
  }
}
...
function createItemsFile(scripts) {
  for (i = 0; i < scripts.length; i++) {
    ...
    writeFile(JSON.stringify(filecontents, null, 4), "SR5-Community-Items", name);
  }
}